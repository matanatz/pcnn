...
from segprovider import SegmentationProvider
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(FLAGS.gpu)):
            ...
            network = SegNetwork(conf.get_config('network'))
        ...
        train_file_list = pv.getTrainDataFiles()
        num_train_file = len(train_file_list)
        test_file_list = pv.getValDataFiles()
        ...
        def train_one_epoch(train_file_idx, epoch_num):
            ...
            for i in range(num_train_file):
                ...
                cur_data, cur_labels, cur_seg = pv.loadDataFile_with_seg(cur_train_filename)
                cur_data, cur_labels, order = pv.shuffle_data(cur_data, np.squeeze(cur_labels))
                cur_data = pv.translate_point_cloud(cur_data)
        ...
        def eval_one_epoch(epoch_num):
            ...
            for i in range(num_test_file):
                ...
                cur_data, cur_labels, cur_seg = pv.loadDataFile_with_seg(cur_test_filename)
